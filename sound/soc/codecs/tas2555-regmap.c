...
static ssize_t tas2555_ping_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	...
	dev_info(pTAS2555->dev, "TAS2555 PGID: 0x%02x\n", n);

	if (nResult == 0)
		return sprintf(buf, "Ping Pass (PGID: 0x%02x)\n", n);
	else
		return sprintf(buf, "Ping Failed\n");
}

static DEVICE_ATTR(ping, 0644, tas2555_ping_show, NULL);
	...
	const struct i2c_device_id *pID)
{
	...
	nResult = device_create_file(pTAS2555->dev, &dev_attr_ping);
	if (nResult < 0) {
		dev_err(pTAS2555->dev, "create file error\n");
	}
