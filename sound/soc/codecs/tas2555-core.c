...
#define TAS2555_CAL_NAME    "/data/nvram/media/tas2555_cal.bin"
...
int tas2555_set_sampling_rate(struct tas2555_priv *pTAS2555, unsigned int nSamplingRate)
{
	...
	if (get_cei_hw_id() <= 2) { /*Use 48K BCLK for voice on EVT, DVT1, DVT2*/
		dev_info(pTAS2555->dev, "Use 48K BCLK for voice on EVT, DVT1, DVT2\n");
		pConfiguration = &(pTAS2555->mpFirmware->mpConfigurations[pTAS2555->mnCurrentConfiguration]);
		if ((nSamplingRate == 48000) && (pTAS2555->mnCurrentConfiguration != 0)) {
			nConfiguration = 0;
			dev_dbg(pTAS2555->dev, "Sampling rate is %d, change to Configuration %d\n",
				nSamplingRate, nConfiguration);
			ret = tas2555_load_configuration(pTAS2555, nConfiguration, false);
		} else if (((nSamplingRate == 16000) || (nSamplingRate == 8000))
			&& (pTAS2555->mnCurrentConfiguration != 1)) {
			nConfiguration = 1;
			dev_dbg(pTAS2555->dev, "Sampling rate is %d, change to Configuration %d\n",
				nSamplingRate, nConfiguration);
			ret = tas2555_load_configuration(pTAS2555, nConfiguration, false);
		} else if ((nSamplingRate == 44100) && (pTAS2555->mnCurrentConfiguration != 5)) {
			nConfiguration = 5;
			dev_dbg(pTAS2555->dev, "Sampling rate is %d, change to Configuration %d\n",
				nSamplingRate, nConfiguration);
			ret = tas2555_load_configuration(pTAS2555, nConfiguration, false);
		} else {
			dev_dbg(pTAS2555->dev, "Sampling rate is %d, keep current Configuration %d\n",
				nSamplingRate, pTAS2555->mnCurrentConfiguration);
		}
	} else { /*Use 16K MCLK for voice after PVT*/
		dev_info(pTAS2555->dev, "Use 16K MCLK for voice after PVT\n");
		pConfiguration = &(pTAS2555->mpFirmware->mpConfigurations[pTAS2555->mnCurrentConfiguration]);
...
check:
	...
	if ((nResult >= 0) && (pBlock->mnType == TAS2555_BLOCK_PLL)) {
	...
	char *pFileName)
{
	...
	fs = get_fs();
	set_fs(KERNEL_DS);
	nFile = sys_open(pFileName, O_RDONLY, 0444);
...
int tas2555_parse_dt(struct device *dev, struct tas2555_priv *pTAS2555)
{
	...
	pTAS2555->mnResetGPIO = RESET_GPIO;
