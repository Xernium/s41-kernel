...
//CEI add start 20130312
int CCI_debug_for_stk3x1x = 0;
//CEI add end 20130312
...
//CEI add probe log node start
char result_lines_probe[2048];
int result_lines_index = 0;
//CEI add probe log node end
...
static uint32_t stk3x1x_get_als_reading_AVG(int sSampleNo) {
	...
	while (DataCount < sSampleNo)
	{
		...
		if ((err = stk3x1x_read_als(stk3x1x_obj->client, &stk3x1x_obj->als))) {
			APS_ERR("[STK] stk3x1x_read_als adc fail\n");
			}
		else {
			ALSData = stk3x1x_obj->als;
			if (CCI_debug_for_stk3x1x)
				APS_ERR("[STK] als adc code = %d\n", ALSData);
			sAveAlsData +=  ALSData;
			DataCount++;
			}
	}
...
int stk3x1x_cci_als_cali(int sSampleNo)
{
	...
	APS_ERR("[STK]stk3x1x_cci_als_cali\n");
	if (!stk3x1x_obj)
	{
	...
	//do ALS cali
	if (als_reading == 0) {
		APS_LOG("[STK]%s als_reading = 0", __func__);
		return -1;
	}

	//if ALS out of range, dont do calibration
	if (als_reading <= 0 || als_reading >= 65535) {
		APS_ERR("[STK]stk3x1x_cci_als_cali() ALS out of range, als_reading = %d\n", als_reading);
		return -1;
	}
	...
	if (cci_transmittance_cali == 0)
		cci_transmittance_cali = 1100; //default transmittance
	APS_ERR("[STK]stk3x1x_cci_als_cali() cci_transmittance_cali = %d, als_reading = %d\n", cci_transmittance_cali, cci_als_adc_cali);

	return 1;
}
//CEI add end 20131022
/*----------------------------------------------------------------------------*/
...
static int stk3x1x_validate_n_handle(struct i2c_client *client)
{
	...
	if ((err = stk3x1x_write_sw_reset(client))) { /* do sw_reset */
		APS_ERR("stk software reset error, err=%d", err);
		return err;
	}
	...
	if ((err = stk3x1x_init_all_reg(client))) { /* do sw_init */
		APS_ERR("stk software init error, err=%d", err);
		return err;
	}
	return 0;
}
...
// CCI add First boot H/L thd for debug start
	if (enable) {
		APS_LOG("%s: show first boot H/L THD => FIRST_BOOT_H_THD = %d, FIRST_BOOT_L_THD = %d\n", __func__, FIRST_BOOT_H_THD, FIRST_BOOT_L_THD);
		APS_LOG("%s: show factory H/L THD => cci_ps_high_thd = %d, cci_ps_low_thd = %d, cci_result_ct = %d\n", __func__, cci_ps_high_thd, cci_ps_low_thd, cci_result_ct);
		}
// CCI add First boot H/L thd for debug end
// CCI read H_thd/L_thd when PS enable/disable start
	...
	APS_LOG("%s: read H/L THD from register before auto cal => H_THD = %2X, %2X, L_THD = %2X, %2X\n", __func__, rbuf[0], rbuf[1], rbuf[2], rbuf[3]);
// CCI read H_thd/L_thd when PS enable/disable end
	...
	if (enable)
	{
		...
		APS_ERR("[STK]stk3x1x_enable_ps() reg_wait = %d\n", reg_wait);
		if (err_wait < 0)
		{
...
*/
	...
	else
	{
		...
		if (1 == obj->hw->polling_mode_ps)
			wake_unlock(&ps_lock);
		/*disable TP before PS detect start*/
		APS_LOG("[STK] PS disablel, enable TP\n");
		disable_pull_status_bar_by_Y_axis(0);
		/*disable TP before PS detect end*/
	}
	...
	if (enable)
	{
		...
		else
		{
			...
			else if (stk3x1x_obj->hw->polling_mode_ps == 0)
			{
				...
				APS_LOG("%s:ps raw 0x%x -> value 0x%x\n", __func__, obj->ps, near_far_state);
				/*disable TP before PS detect start*/
				if (near_far_state == 0) { /*PS detect*/
					disable_pull_status_bar_by_Y_axis(1);
					APS_LOG("[STK] disable TP\n");
					}
				else { /*PS undetect*/
					disable_pull_status_bar_by_Y_axis(0);
					APS_LOG("[STK] enable TP\n");
					}
				/*disable TP before PS detect end*/
				...
				if (err != 0)//hwmsen_get_interrupt_data
				{
					...
					/*disable TP before PS detect start*/
					APS_LOG("[STK] ps_report_interrupt_data fail, enable TP\n");
					disable_pull_status_bar_by_Y_axis(0);
					/*disable TP before PS detect end*/
				}
			}
	...
	else //STK add for CCI start 20130430 dust issue for variation in 2 consequential calls
	{
		...
		else
		{
			...
			{
				APS_ERR("[STK] use hang_up_hthd = %d, hang_up_lthd = %d\n", hang_up_hthd, hang_up_lthd);
...
static int stk_ps_tune_zero_final(struct stk3x1x_priv *obj)
{
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "FIRST_BOOT_H_THD = %d\n", FIRST_BOOT_H_THD);
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "FIRST_BOOT_L_THD = %d\n", FIRST_BOOT_L_THD);
	// CCI add First boot H/L thd for debug end
	if ((err = stk3x1x_write_ps_high_thd(obj->client, atomic_read(&obj->ps_high_thd_val))))
	{
		APS_ERR("write high thd error: %d\n", err);
...
static int32_t stk_tune_zero_get_ps_data(struct stk3x1x_priv *obj)
{
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "%s: ps #%d=%d\n", __func__, obj->data_count, obj->ps);
...
static int stk3x1x_ps_tune_zero_val(void)
{
	...
	{
		APS_ERR("unsupported PS_IT(0x%x)\n", mode);
		return -1;
	}
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if (ret == 0)
	{
		...
		if (word_data > obj->psa)
		{
			if (obj->psa == 0)
			{
				temp_max_min_diff = word_data >> 1;
				APS_LOG("[STK] temp_max_min_diff = %d\n", temp_max_min_diff);
			}
			obj->psa = word_data;
	...
	if (diff > STK_MAX_MIN_DIFF && diff > temp_max_min_diff)
	{
		...
		{
			APS_ERR("[STK] use STK default delta value HT = %d LT = %d\n", obj->psi_set + STK_HT_N_CT, obj->psi_set + STK_LT_N_CT);
		...
		else
		{
			...
			APS_ERR("[STK] use FTM delta value HT = %d, LT = %d\n",
					obj->psi_set + cci_ps_high_thd - fac_ct - offset_H,
					obj->psi_set + cci_ps_low_thd - fac_ct - offset_L);
...
static void stk3x1x_eint_work(struct work_struct *work)
{
	...
	if (((1<<STK_BIT_PS) &  obj->pending_intr) && (obj->hw->polling_mode_ps == 0))
	{
		...
		APS_LOG("%s: read H/L THD from register when interrupt trigger => H_THD = %2X, %2X, L_THD = %2X, %2X\n", __func__, rbuf[0], rbuf[1], rbuf[2], rbuf[3]);
		...
		/*disable TP before PS detect start*/
				if (near_far_state == 0) { /*PS detect*/
					...
					APS_LOG("[STK] set delay before enable TP\n");
					mdelay(30);
					disable_pull_status_bar_by_Y_axis(0);
					APS_LOG("[STK] enable TP\n");
					}
...
static int stk3x1x_init_client(struct i2c_client *client)
{
	...
	u8 ps_ctrl = 0;
	//u8 int_status;
result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "stk3x1x_init_client++\n");
	if ((err = stk3x1x_write_sw_reset(client)))
	{
		APS_ERR("software reset error, err=%d", err);
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "software reset error, err=%d\n", err);
		//return err;
		return -1;
	}
	else
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "software reset OK\n");
	...
	if (err < 0)
		...
		printk("[stk] check sensor type!\n");
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "[stk] check sensor type!\n");
		}
	else
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "check sensor type OK\n");

	result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "polling_mode_ps = %d\n", obj->hw->polling_mode_ps);
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "polling_mode_als = %d\n", obj->hw->polling_mode_als);
	if (obj->hw->polling_mode_ps == 0 || obj->hw->polling_mode_als == 0)
	{
		if ((err = stk3x1x_setup_eint(client)))
		{
			APS_ERR("setup eint error: %d\n", err);
			result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "setup eint error: %d\n", err);
			printk("stk err  = %d\n", err);
			return err;
		}
		else
			result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "setup eint OK\n");
	}

	if ((err = stk3x1x_write_state(client, atomic_read(&obj->state_val))))
	{
		APS_ERR("write stete error: %d\n", err);
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write stete error: %d\n", err);
		return err;
	}
	else
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write stete OK\n");
	...
	*/
	...
	{
		APS_ERR("write ps error: %d\n", err);
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write ps error: %d\n", err);
		return err;
	}
	else
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write ps(0x%X) OK\n", ps_ctrl);

	if ((err = stk3x1x_write_als(client, atomic_read(&obj->alsctrl_val))))
	{
		APS_ERR("write als error: %d\n", err);
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write als error: %d\n", err);
		return err;
	}
	else
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write als(0x%X) OK\n", atomic_read(&obj->alsctrl_val));

	if ((err = stk3x1x_write_led(client, obj->ledctrl_val)))
	{
		APS_ERR("write led error: %d\n", err);
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write led error: %d\n", err);
		return err;
	}
	else
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write led(0x%X) OK\n", obj->ledctrl_val);

	if ((err = stk3x1x_write_wait(client, obj->wait_val)))
	{
		APS_ERR("write wait error: %d\n", err);
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write wait error: %d\n", err);
		return err;
	}
	else
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write wait(0x%X) OK\n", obj->wait_val);
#ifndef STK_TUNE0
	if ((err = stk3x1x_write_ps_high_thd(client, atomic_read(&obj->ps_high_thd_val))))
	{
		APS_ERR("write high thd error: %d\n", err);
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write high thd error: %d\n", err);
		return err;
	}
	else
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write high thd OK\n");

	if ((err = stk3x1x_write_ps_low_thd(client, atomic_read(&obj->ps_low_thd_val))))
	{
		APS_ERR("write low thd error: %d\n", err);
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write low thd error: %d\n", err);
		return err;
	}
	else
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write low thd OK\n");
#endif
	if ((err = stk3x1x_write_int(client, obj->int_val)))
	{
		APS_ERR("write int mode error: %d\n", err);
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write int mode error: %d\n", err);
		return err;
	}
	else
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "write int mode(0x%X) OK\n", obj->int_val);
#ifdef STK_FIR
...
static ssize_t stk3x1x_store_cali(struct device_driver *ddri, const char *buf, size_t size)
{
	...
	printk(KERN_INFO "%s [STK]cci_ps_high_thd = %d, cci_ps_low_thd = %d", __func__, cci_ps_high_thd, cci_ps_low_thd);
...
/*[Bug1147] modify H_THD, only for CT41 end*/
/*H/L spec: H-L > 20, L-CT > 20*/
	if (cci_ps_high_thd > cci_ps_low_thd + 40)
		cci_ps_high_thd = (cci_ps_high_thd + cci_ps_low_thd)/2;
	else
		cci_ps_high_thd = cci_ps_low_thd + 20;
	APS_LOG("[STK] modify H_THD, cci_ps_high_thd = %d", cci_ps_high_thd);
/*[Bug1147] modify H_THD, only for CT41 end*/
...
static ssize_t stk3x1x_show_ccidebug(struct device_driver *ddri, char *buf)
{
	...
	APS_ERR("[STK]stk3x1x_show_ccidebug\n");
	APS_ERR("[STK]CCI_debug_for_stk3x1x = %d\n", CCI_debug_for_stk3x1x);
...
static ssize_t stk3x1x_store_ccidebug(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	APS_ERR("[STK]stk3x1x_store_ccidebug\n");
	if (1 != sscanf(buf, "%d", &enable))
	{
		APS_ERR("[STK]invalid format: '%s'\n", buf);
		return -1;
	}
	APS_ERR("[STK]stk3x1x_store_ccidebug enable = %d", enable);

	if (enable) {
		CCI_debug_for_stk3x1x = 1;
		APS_ERR("[STK]stk3x1x_store_ccidebug set enable");
		}
	else {
		CCI_debug_for_stk3x1x = 0;
		APS_ERR("[STK]stk3x1x_store_ccidebug set disable");
		}
	return count;
}
...
static uint32_t stk3x1x_get_ps_reading_AVG(int sSampleNo) {
	...
	while (DataCount < sSampleNo)
	{
		...
		if (err != 0)
			{
			APS_ERR("[STK]stk3x1x_read_ps fail\n");
			}
		else {
			PSData = stk3x1x_obj->ps;
			APS_ERR("[STK]ps code = %d\n", PSData);
			sAvePsData +=  PSData;
			DataCount++;
			}
	}
...
static ssize_t stk3x1x_show_GC_HT(struct device_driver *ddri, char *buf)
{
	...
	//---set default THD as ps_thd_l = 10 first then, ps_thd_h = 30,  to ensure int trigger start---
	atomic_set(&stk3x1x_obj->ps_low_thd_val, 10);
	...
	atomic_set(&stk3x1x_obj->ps_high_thd_val, 30);
	...
	//------set default THD as ps_thd_h = 30, ps_thd_l = 10 to ensure int trigger end---
	msleep(150);
...
static ssize_t stk3x1x_show_GC_LT(struct device_driver *ddri, char *buf)
{
	...
	APS_ERR("[STK]P sensor start calibrating Low threshold\n");
	msleep(150);
	reading = stk3x1x_get_ps_reading_AVG(5);//get ps code for 5 times;
...
static ssize_t stk3x1x_show_GC_CT(struct device_driver *ddri, char *buf)
{
	...
	printk("[STK] P sensor stk calibrate CT---\n cci_ps_high_thd = %d, cci_ps_low_thd = %d\n Diff_ThdH_L = %d, Diff_ThdL_CT = %d\n", cci_ps_high_thd, cci_ps_low_thd, Diff_ThdH_L, Diff_ThdL_CT);
		...
		if ((cci_result_ct > 0) && (cci_result_ct < 1600) && (Diff_ThdL_CT > 20) && (Diff_ThdH_L > 20)) { /* P sensor Criteria */
			atomic_set(&stk3x1x_obj->ps_high_thd_val, cci_ps_high_thd);
			atomic_set(&stk3x1x_obj->ps_low_thd_val, cci_ps_low_thd);
		if ((err = stk3x1x_write_ps_high_thd(stk3x1x_obj->client, atomic_read(&stk3x1x_obj->ps_high_thd_val))))
		{
...
static ssize_t stk3x1x_show_TEST(struct device_driver *ddri, char *buf)
{
	...
	APS_ERR("[STK]P sensor stk start test.\n");
	reading = stk3x1x_get_ps_reading_AVG(5);
...
static ssize_t stk3x1_cali_Light(struct device_driver *ddri, char *buf)
{
	...
	APS_ERR("[STK]L sensor stk start calibrate.\n");
	...
	result = (ret && (cci_als_adc_cali >= 220) && (cci_als_adc_cali <= 560)) ? 1:0; /* TBD */
...
//CEI add probe log node start
static ssize_t probe_log_show(struct device_driver *ddri, char *buf)
{
...
static ssize_t stk3x1x_test_Light(struct device_driver *ddri, char *buf)
{
	...
	result = (cci_als_value >= 255 && cci_als_value <= 345) ?  true : false; /* criteria for DVT1: 255~345 */
...
//CEI add probe log node start
static DRIVER_ATTR(probe_log, S_IWUSR | S_IRUGO, probe_log_show, NULL);
//CEI add probe log node end
#ifdef STK_TUNE0
...
#endif
{
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_ALS_RAW_DATA:
			...
			break;
		//CEI add start 20130110
// add for FTM interrupt check 20130416 start

		case ALSPS_SET_THD:
			APS_ERR("ALSPS_SET_THD enter");
			...
			APS_ERR("ps_high_thd = %d, ps_low_thd = %d", ftm_thd.ps_high_thd, ftm_thd.ps_low_thd);
			if ((err = stk3x1x_write_ps_high_thd(client, atomic_read(&obj->ps_high_thd_val))))
			{
		...
		case ALSPS_GET_INTERRUPT_COUNT:
			APS_ERR("ALSPS_GET_INTERRUPT_COUNT enter CCI_FTM_INTERRUPT = %d", CCI_FTM_INTERRUPT);
...
// add for FTM interrupt check 20130416 end
		...
		case ALSPS_IOCTL_SET_CALI_CCI:
			...
			APS_ERR("ALSPS_IOCTL_SET_CALI_CCI cci_ps_high_thd = %d, cci_ps_low_thd = %d\n", cci_ps_high_thd, cci_ps_low_thd);
			...
			APS_ERR("ALSPS_IOCTL_SET_CALI_CCI cci_result_ct = %d, cci_ps_cover = %d, cci_als_value = %d  cci_als_adc_test =%d\n", cci_result_ct, cci_ps_cover, cci_als_value, cci_als_adc_test);
			...
			APS_ERR("ALSPS_IOCTL_SET_CALI_CCI als_transmittance_cali = %d, als_adc_cali = %d cci_als_lux_cali = %d\n", cci_transmittance_cali, cci_als_adc_cali, cci_als_lux_cali);
		...
		case ALSPS_SET_AUTO_CALI:
			APS_ERR("%s ALSPS_SET_AUTO_CALI enter", __func__);
			if (copy_from_user(&ftm_auto_cali, ptr, sizeof(ftm_auto_cali))) {
				err = -EFAULT;
				goto err_out;
			}
			if (ftm_auto_cali) {
				APS_ERR("%s ALSPS_SET_AUTO_CALI disable auto calibration", __func__);
				stk3x1x_write_state(obj->client, 0x03);
			}
			else{
				APS_ERR("%s ALSPS_SET_AUTO_CALI enable auto calibration", __func__);
				stk3x1x_write_state(obj->client, 0x43);
			}
			break;
#endif
		/*----------------------------------for factory mode test---------------------------------------*/
		//CEI add end 20130110
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "%s: driver version: %s\n", __func__, DRIVER_VERSION);
	...
	if (obj->irq_node != NULL) {
		printk("stk irq !=null\n");
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "irq_node check OK\n");
		}
	else {
		printk("stk irq ==null\n");
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "irq_node check FAIL\n");
		}
	...
	if (obj->hw->polling_mode_ps == 0)
	{
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "%s: enable PS interrupt\n", __func__);
	}
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "%s: enable ALS interrupt\n", __func__);
	}
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "%s: state_val=0x%x, psctrl_val=0x%x, alsctrl_val=0x%x, ledctrl_val=0x%x, wait_val=0x%x, int_val=0x%x\n",
		__func__, atomic_read(&obj->state_val), atomic_read(&obj->psctrl_val), atomic_read(&obj->alsctrl_val),
		obj->ledctrl_val, obj->wait_val, obj->int_val);
	...
	//result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "stk obj->als_level_num  = %d , obj->als_value_num = %d\n",obj->als_level_num,obj->als_value_num);
...
//	client->timing=400;
	...
	if ((err = misc_register(&stk3x1x_device)))
	{
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "stk3x1x_device register failed\n");
	...
	if (err == 0) {
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "stk3x1x_init_client success\n");
	}
	else if (err == -1) {
		APS_ERR("stk3x1x_init_client err_init_client_before_irq_err\n");
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "stk3x1x_init_client err_init_client_before_irq_err\n");
			goto err_init_client_before_irq_err;
	}
	else if (err == -2) {
		APS_ERR("stk3x1x_init_client err_irq_node_error\n");
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "stk3x1x_init_client err_irq_node_error\n");
			goto err_irq_node_error;
	}
	else if (err == -3) {
		APS_ERR("stk3x1x_init_client err_irq_parse_map_err\n");
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "stk3x1x_init_client err_irq_parse_map_err\n");
			goto err_irq_parse_map_err;
	}
	else if (err == -4) {
		APS_ERR("stk3x1x_init_client err_irq_request_err\n");
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "stk3x1x_init_client err_irq_request_err\n");
			goto err_irq_request_err;
	}
	else	{
		APS_ERR("stk3x1x_init_client invalid err(%dx)\n", err);
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "%s:stk3x1x_init_client invalid err(%d)\n", __func__, err);
			goto err_init_client_err;
	}
	...
	if (err) {
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "register fail = %d\n", err);
	...
	if (err) {
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "tregister fail = %d\n", err);
		goto exit_sensor_obj_attach_fail;
	}
	...
	ps_ctl.is_report_input_direct = true; /* set psensor as interrupt mode in alsps.c */
...
err = stk3x1x_create_attr(&stk3x1x_init_info.platform_diver_addr->driver);
	if (err < 0)
	{
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "%s:could not create sysfs group for als\n", __func__);
...
/*--------------------------device node dynamic link create--------------------------------------*/
	...
	if (err < 0) {
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "could not create sysfs link for als\n");
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "stk3x1x_i2c_probe() OK!\n");
	return 0;
...
err_irq_node_error:
err_init_client_before_irq_err:
...
/*--------------------------device node create when probe fail------------------------------------------------*/
	err = stk3x1x_create_attr(&stk3x1x_init_info.platform_diver_addr->driver);
	if (err < 0)
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "%s:[Probe Fail]could not create sysfs group for als\n", __func__);
	else {
		alsps_sysfs_link = kobject_create_and_add("alsps", NULL);
		...
		if (err < 0)
			result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "[Probe Fail]could not create sysfs link for als\n");
	}
/*--------------------------device node create when probe fail------------------------------------------------*/

	APS_ERR("probe FAIL, return -1\n");
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], sizeof(result_lines_probe) - result_lines_index, "%s: probe FAIL, return -1\n", __func__);
	return -1;
}
