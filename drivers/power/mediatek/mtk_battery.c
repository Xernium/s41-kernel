...
static int fixed_bat_per = 0xffff;
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		if (fixed_bat_per == 0xffff)
			val->intval = data->BAT_CAPACITY;
		else
			val->intval = fixed_bat_per;
		break;
...
static ssize_t show_Battery_Percentage(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	bm_err("show_Battery_Percentage: %d %d\n", battery_main.BAT_CAPACITY, fixed_bat_per);
	return sprintf(buf, "%d\n", fixed_bat_per);
}

static ssize_t store_Battery_Percentage(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t size)
{
	signed int temp;
	...
	if (kstrtoint(buf, 10, &temp) == 0) {
		fixed_bat_per = temp;
		bm_err("store_Battery_Temperature: fixed_bat_tmp:%d ,tmp:%d!\n", temp, battery_main.BAT_CAPACITY);
		battery_update(&battery_main);
	} else {
		bm_err("store_Battery_Temperature: format error!\n");
	}
...
static DEVICE_ATTR(Battery_Percentage, 0664, show_Battery_Percentage,
		store_Battery_Percentage);
...
static ssize_t show_FG_Current(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (current_sign == true)
		fg_current_inout_battery = 0 - ichg;
	else
		fg_current_inout_battery = ichg;

	pr_err("[FG_Current] gFG_current_inout_battery : %d\n", fg_current_inout_battery);
	return sprintf(buf, "%d\n", fg_current_inout_battery);
}
...
/*****************************/
	...
	ret = device_create_file(&(dev->dev), &dev_attr_Battery_Percentage);
