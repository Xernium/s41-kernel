...
static bool high_temp_occur;
static bool low_temp_occur;
	...
	if (info->battery_temperature >= info->data.temp_t4_threshold) {
		...
		if (((sw_jeita->sm == TEMP_T3_TO_T4)
			...
			if (sw_jeita->sm == TEMP_T1_TO_T2) {
				pr_err("[SW_JEITA] Battery Normal Temperature recoverd from low temp !!\n");
...
static void mtk_battery_notify_VBatTemp_check(struct charger_manager *info)
{
	...
	if (high_temp_occur && (info->battery_temperature < info->thermal.max_charge_temperature_minus_x_degree)) {
		high_temp_occur = false;
		pr_err("[BATTERY] bat_temp (%d) below the high threshold, start charging and dismiss the dialog\n",
				info->battery_temperature);
	} else if (low_temp_occur && (info->battery_temperature >
			info->thermal.min_charge_temperature_plus_x_degree)) {
		low_temp_occur = false;
		pr_err("[BATTERY] bat_temp (%d) above the low threshold, start charging and dismiss the dialog\n",
				info->battery_temperature);
	}
#endif
}
...
static ssize_t show_charger_ic_ping(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned int chg_hw_eixst = 0;
	struct charger_manager *pinfo = dev->driver_data;

	chg_hw_eixst = charger_dev_ping_ic(pinfo->chg1_dev);
	pr_err("%s: chg_ic_exist = %d\n", __func__, chg_hw_eixst);

	charger_dev_dump_registers(pinfo->chg1_dev);
	return sprintf(buf, "%u\n", chg_hw_eixst);
}

static ssize_t store_charger_ic_ping(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t size)
{
...
static DEVICE_ATTR(Charger_IC_Ping, 0664, show_charger_ic_ping, store_charger_ic_ping);
						...
						size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d", &cmd_current_unlimited, &cmd_discharging) == 2) {
		...
		if (cmd_discharging == 1) {
			...
			charger_dev_enable_cfo(info->chg1_dev, false);
			charger_manager_notifier(info, CHARGER_NOTIFY_STOP_CHARGING);
		} else if (cmd_discharging == 0) {
			info->cmd_discharging = false;
			charger_dev_enable(info->chg1_dev, true);
			charger_dev_enable_cfo(info->chg1_dev, true);
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_Charger_IC_Ping);
	if (ret)
		goto _out;
